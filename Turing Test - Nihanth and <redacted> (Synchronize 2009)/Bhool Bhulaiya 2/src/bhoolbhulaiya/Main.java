...
public class Main extends JFrame {
    ...
    public Main() {
        ...
        okButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (isThereAPath(true, 0, 0)) {
                    btns[0][0].setEnabled(false);
    ...
    boolean isThereAPath(boolean orig, int x, int y) {
        while (orig) {
            try {
...
//        }
        if (returnval && orig) {
            while (it.hasNext()) {
                Point p = (Point) it.next();
                btns[(int) p.getX()][(int) p.getY()].setText("1");
                if (!isThereAPath(false, x, y)) {
                    btns[(int) p.getX()][(int) p.getY()].setEnabled(false);
                }
                btns[(int) p.getX()][(int) p.getY()].setText("0");
