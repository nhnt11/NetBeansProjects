...
public class Viking implements Runnable {
    ...
    /* Stores the nick that is finally used
     * though only for convenience during initialization.
     * Not used thereafter. */
    ...
    private static final HashMap<String, Integer> mCommands = new HashMap();
    /* HashMap used to store syntax for each command.
     * For consistency purposes. */
    private static final HashMap<Integer, String> mCommandsSyntax = new HashMap();

    static {
        mCommands.put("exec", EXEC);
        mCommands.put("holdkeys", KEY_HOLD);
        mCommands.put("releasekeys", KEY_RELEASE);
        mCommands.put("releaseallkeys", KEY_RELEASE_ALL);
        mCommands.put("presskeys", KEY_PRESS);
        mCommands.put("type", TYPE);
        mCommands.put("movemouse", MOVE_MOUSE);
        mCommands.put("movemouserel", MOVE_MOUSE_REL);
        mCommands.put("pressmouse", PRESS_MOUSE);
        mCommands.put("releasemouse", RELEASE_MOUSE);
        mCommands.put("clickmouse", CLICK_MOUSE);
        mCommands.put("copyfile", COPY_FILE);
        mCommands.put("movefile", MOVE_FILE);
        mCommands.put("deletefile", DELETE_FILE);
        mCommands.put("getcb", GETCB);
        mCommands.put("macro", MACRO);
        mCommands.put("echo", ECHO);
        mCommands.put("sleep", SLEEP);
        mCommands.put("dumplog", DUMPLOG);
        mCommands.put("exit", EXIT);
        mCommands.put("help", HELP);

        mCommandsSyntax.put(EXEC, "Usage - exec [executable]");
        mCommandsSyntax.put(KEY_HOLD, "Usage - holdkeys [keystroke]");
        mCommandsSyntax.put(KEY_RELEASE, "Usage - releasekeys [keystroke]");
        mCommandsSyntax.put(KEY_RELEASE_ALL, "Usage - releaseallkeys");
        mCommandsSyntax.put(KEY_PRESS, "Usage - presskeys [keystroke]");
        mCommandsSyntax.put(TYPE, "Usage - type [text]");
        mCommandsSyntax.put(MOVE_MOUSE, "Usage - movemouse [x] [y]");
        mCommandsSyntax.put(MOVE_MOUSE_REL, "Usage - movemouserel [relative x] [relative y]");
        mCommandsSyntax.put(PRESS_MOUSE, "Usage - pressmouse [button]");
        mCommandsSyntax.put(RELEASE_MOUSE, "Usage - releasemouse [button]");
        mCommandsSyntax.put(CLICK_MOUSE, "Usage - clickmouse [button]");
        mCommandsSyntax.put(COPY_FILE, "Usage - copyfile [file] [dest]");
        mCommandsSyntax.put(MOVE_FILE, "Usage - movefile [file] [dest]");
        mCommandsSyntax.put(DELETE_FILE, "Usage - deletefile [file]");
        mCommandsSyntax.put(GETCB, "Usage - getcb");
        mCommandsSyntax.put(MACRO, "Usage - macro [macroname] [arguments]");
        mCommandsSyntax.put(ECHO, "Usage - echo [text]");
        mCommandsSyntax.put(SLEEP, "Usage - sleep [millis]");
        mCommandsSyntax.put(DUMPLOG, "Usage - dumplog");
        mCommandsSyntax.put(EXIT, "Usage - exit");
...
//    };
    ...
    /*
     * Three threads are used for shell mode -
     * mOutputThread, mErrorThread, and mTerminatorThread.
     * Each has its own runnable, which is used to reinitialize the thread
     * every time a new command is executed.
     * This is because Java does not allow re-starting a thread without
     * reinitializing.
     *
...
//            mUserInputMon.addUserInputListener(mUserIRCClient);
     ...
     * Send PRIVMSG's to be further processed if they are from mCurrentAuthorizedMaster
     * Also displays NOTICEs from the server. All other messages ignored. */
     ...
     * returns false if no process is currently running or
     * true if message was successfully passed to the process */
    ...
    /* Process PRIVMSG from a user (processServerMessage() ensures the user is authenticated)
     * The first word of the message is taken as the keyword using a StringTokenizer.
     * The keyword is lookup()'d and the corresponding action is taken. */
    private void processPrivMsg(String msg) throws IOException {
        StringTokenizer st = new StringTokenizer(msg, " ");
        String keyword = st.nextToken();
        int command = lookup(keyword);
        switch (command) {
            case EXEC:
            ...
            case KEY_HOLD:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), holdKeys(st.nextToken()));
                break;
            case KEY_RELEASE:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), releaseKeys(st.nextToken()));
                break;
            ...
            case KEY_PRESS:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), keyPress(st.nextToken()));
                break;
            case TYPE:
                if (msg.trim().length() < keyword.length() + 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), type(
                            msg.substring(keyword.length() + 1)));
                break;
            case MOVE_MOUSE:
                if (st.countTokens() != 2)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), moveMouse(st.nextToken(),
                            st.nextToken()));
                break;
            case MOVE_MOUSE_REL:
                if (st.countTokens() != 2)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(),
                            moveMouseRel(st.nextToken(), st.nextToken()));
                break;
            case PRESS_MOUSE:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), pressMouse(st.nextToken()));
                break;
            case RELEASE_MOUSE:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), releaseMouse(st.nextToken()));
                break;
            case CLICK_MOUSE:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), clickMouse(st.nextToken()));
                break;
            case COPY_FILE:
                if (st.countTokens() != 2)
                    sendInvalidSyntaxMessage(command);
            ...
            case DELETE_FILE:
                if (st.countTokens() != 1)
                    sendInvalidSyntaxMessage(command);
                else sendMessage(getCurrentAuthorizedNick(), deleteFile(st.nextToken()));
                break;
            case GETCB:
            ...
            case MACRO:
                ...
                try {
                    ...
                    if (argline.length() < 1) args = new String[0];
                    else args = argline.split(" +");
    ...
    private void sendInvalidSyntaxMessage(int command) throws IOException {
        sendMessage(getCurrentAuthorizedNick(), mCommandsSyntax.get(command));
    }

    /*
     * Executes a command.
     * Starts the three shell threads and puts the app into "shell mode"
     * - all further commands are transferred as input to the shell process
     * until it terminates.
     */
    ...
    private String holdKeys(String keyCombo) {
        if (keyCombo.trim().equals(""))
            return mCommandsSyntax.get(KEY_HOLD);
    ...
    private String releaseKeys(String keyCombo) {
        if (keyCombo.trim().equals(""))
            return mCommandsSyntax.get(KEY_RELEASE);
        if (robotNotSupported)
            return "Keyboard emulation not supported on your OS.";
    ...
    private String keyPress(String keyCombo) {
        if (keyCombo.trim().equals(""))
            return mCommandsSyntax.get(KEY_PRESS);
        if (robotNotSupported)
            return "Keyboard emulation not supported on your OS.";
    ...
    private String moveMouse(String X, String Y) {
        ...
        try {
            ...
            return mCommandsSyntax.get(MOVE_MOUSE);
        }
    ...
    private String moveMouseRel(String relX, String relY) {
        ...
        try {
            ...
            return mCommandsSyntax.get(MOVE_MOUSE_REL);
        }
    ...
    private String pressMouse(String button) {
        ...
        return mRobot.mousePress(button)
                ...
                : mCommandsSyntax.get(PRESS_MOUSE);
    }
    ...
    private String releaseMouse(String button) {
        ...
        return mRobot.mouseRelease(button)
                ...
                : mCommandsSyntax.get(RELEASE_MOUSE);
    }
    ...
    private String clickMouse(String button) {
        ...
        return mRobot.mouseClick(button)
                ...
                : mCommandsSyntax.get(CLICK_MOUSE);
    }
    ...
    private void sleep(String timeMillis) throws IOException {
        try {
            ...
            sendMessage(getCurrentAuthorizedNick(), mCommandsSyntax.get(SLEEP));
    ...
    private int lookup(String key) {
        Integer i = (Integer) mCommands.get(key);
        return i == null ? -1 : i;
    }
